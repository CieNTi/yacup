...
struct ce_command_codec
{
   ...
   */
  struct ce_command_codec_encoder
  {
    /**
   ...
   * @brief      Structure that defines a `ce` codec encoder operations
   */
  struct ce_command_codec_decoder
  {
    /**
 ...
 * @details    Checks and initializes `ce_command_codec` common data, then calls the
 *             lower level init function passed by argument. The latter is
 *             defined at each `ce_command_codec` unit, and it is where the encoding and
 *             decoding functions are really assigned.
 *
 * @param      codec                 Pointer to a codec to initialize
 * @param[in]  ce_command_codec_driver_init  The ce codec driver initializer function
 *
 * @return     One of:
 *             | Value         | Meaning          |
 *             | :-----------: | :--------------- |
